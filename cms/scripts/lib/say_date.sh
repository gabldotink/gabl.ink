...
say_date(){
  ...
  if [ "$lang_l" = en ];then
    ...
    if   [ "$say_date_y" -lt 100 ] &&
      ...
      printf '<abbr title="anno Domini">AD</abbr>\302\240%s' "$say_date_y"
    elif [ "$say_date_y" -le 0 ];then
      printf '%s\302\240<abbr title="before Christ">BC</abbr>' "$((1+$((-say_date_y))))"
    else
      printf '%s' "$say_date_y"
    fi
  elif [ "$lang_l" = fr ];then
    ...
    if   [ "$say_date_y" -lt 100 ] &&
         [ "$say_date_y" -gt 0 ];then
      printf '%s\302\240<abbr title="après Jésus‐Christ">ap.\302\240J.‑C.</abbr>' "$say_date_y"
    elif [ "$say_date_y" -le 0 ];then
      printf '%s\302\240<abbr title="avant Jésus‐Christ">av.\302\240J.‑C.</abbr>' "$((1+$((-say_date_y))))"
    else
      printf '%s' "$say_date_y"
    fi
  elif [ "$lang_l" = es ];then
    printf '%s\302\240de\302\240' "$say_date_d"
    printf '%s de ' "$say_date_m_html"
    if   [ "$say_date_y" -lt 100 ] &&
         [ "$say_date_y" -gt 0 ];then
      printf '%s\302\240<abbr title="después de Cristo">d.\302\240C.</abbr>' "$say_date_y"
    elif [ "$say_date_y" -le 0 ];then
      printf '%s\302\240<abbr title="antes de Cristo">a.\302\240C.</abbr>' "$((1+$((-say_date_y))))"
    else
      printf '%s' "$say_date_y"
    fi
  elif [ "$lang_l" = ja ] ||
