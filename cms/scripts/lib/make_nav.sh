...
make_nav_f_l(){
  ...
  if [ "$(eval 'printf "%s" "$'"$make_nav_f_l_z"'"')" = null ];then
    ...
    printf -- '<a href="../../%s/%s/" hreflang="%s" type="text/html">' "$(printf '%02d' "$(eval 'printf "%s" "$container_'"$make_nav_f_l_i"'"')")" "$lang" "$lang"
  fi
...
make_nav_p_n(){
  ...
  if [ "$(eval 'printf -- "%s" "$'"$make_nav_p_n_i"'"')" = null ];then
    ...
    printf -- '<a href="../../%s/%s/" rel="%s" hreflang="%s" type="text/html">' "$(printf '%02d' "$(eval 'printf "%s" "$'"$make_nav_p_n_i"'"')")" "$lang" "$make_nav_p_n_i" "$lang"
  fi
